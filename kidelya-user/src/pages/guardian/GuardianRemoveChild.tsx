...
export default function GuardianRemoveChild() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(`/api/guardians/${id}/children`)
        const json = await res.json()
        setChildren(json.children || [])
      } catch (e) {
        console.error(e)
        setError("Impossible de charger les enfants associés.")
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/guardians/${id}/remove-child`, {
        method: "POST",
  ...
  return (
    <div className="min-h-screen bg-[#FFF9F0] px-6 py-10 max-w-xl mx-auto">
      <h1 className="text-3xl font-bold text-[#93197D] mb-8">
        Retirer un enfant de ce parent 🌸
      </h1>
