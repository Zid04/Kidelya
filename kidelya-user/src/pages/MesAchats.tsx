...
function ActivityCard({ a, owned, viaSubscription }: { a: PurchasedActivity; owned: boolean; viaSubscription?: boolean }) {
  ...
  return (
    <Link
      ...
      state={{ from: '/mes-achats' }}
      className="group bg-[#FFFEFA] rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow"
    >
      {img ? (
        <img src={img} alt={a.title} className="h-40 w-full object-cover" />
      ) : (
        <div className="h-40 bg-gradient-to-br from-[#D5CDE2] to-[#E94E6F]/20" />
      )}
      <div className="p-4">
        <h3 className="font-black text-[#273068] mb-1 line-clamp-1">{a.title}</h3>
        <div className="mt-1.5 flex flex-wrap gap-1.5">
          ...
          {a.duration && (
            <span className="rounded-full bg-[#D8EAF2] px-2.5 py-0.5 text-[10px] font-bold text-[#7BA7C0]">
              {a.duration} min
            </span>
          )}
        </div>
        {owned ? (
          <span className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-[#7C67B2]">
...
function PackCard({ p, owned, viaSubscription }: { p: PurchasedPack; owned: boolean; viaSubscription?: boolean }) {
  ...
  return (
    <Link
      to={`/packs/${p.idpack}`}
      className="bg-[#FFFEFA] rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow"
    >
      {img ? (
        <img src={img} alt={p.title} className="h-40 w-full object-cover" />
      ) : (
        <div className="h-40 bg-gradient-to-br from-[#D5CDE2] to-[#F4BB48]/20" />
      )}
      <div className="p-4">
        <h3 className="font-black text-[#273068] mb-1 line-clamp-1">{p.title}</h3>
  ...
  return (
    <div className="min-h-screen bg-white px-4 py-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-black text-[#7C67B2]">Ma bibliothèque d'activités</h1>
      ...
      {/* Barre d'onglets */}
      <div className="flex gap-1 mb-8 bg-[#D5CDE2] rounded-xl p-1">
        {tabs.map(t => (
          <button
            key={t.key}
            ...
            className={`flex-1 py-2 px-3 rounded-lg text-sm font-semibold transition-all ${
              tab === t.key
                ? "bg-[#FFFEFA] text-[#7C67B2] shadow-sm"
      ...
      {tab === "activites" && (
        loading ? (
          <p className="text-center text-[#7C67B2] py-12">Chargement…</p>
        ) : (
          <>
            {allAccess && (
              <div className="flex items-center gap-3 bg-[#D5CDE2] rounded-xl px-4 py-3 mb-6">
                <span className="text-[#7C67B2] text-lg">★</span>
            ...
            {!data || data.activities.length === 0 ? (
              <div className="text-center py-10">
                ...
                <p className="text-[#273068] mb-4">Aucune activité disponible pour l'instant.</p>
              </div>
            ) : (
              <>
                <p className="text-xs font-semibold uppercase tracking-wider text-[#7C67B2] mb-3">
            ...
            {!allAccess && data && data.recommended_activities.length > 0 && (
              <>
                <div className="flex items-center justify-between mb-3 mt-2">
                  <p className="text-xs font-semibold uppercase tracking-wider text-[#7C67B2]">
        ...
        loading ? (
          ...
          <>
            ...
            {!data || data.packs.length === 0 ? (
              <div className="text-center py-10">
                <p className="text-4xl mb-3">📦</p>
                <p className="text-[#273068] mb-4">Aucun pack disponible pour l'instant.</p>
              </div>
            ) : (
              <>
                <p className="text-xs font-semibold uppercase tracking-wider text-[#7C67B2] mb-3">
      ...
      {tab === "abonnement" && (
        plan ? (
          <div className="max-w-md mx-auto">
            <div className="bg-[#FFFEFA] rounded-2xl p-6 shadow-sm">
              <div className="flex items-center gap-3 mb-5">
                <span className="h-12 w-12 flex items-center justify-center rounded-full bg-[#7C67B2] text-white text-xl font-black">
                  ★
                </span>
                <div>
                  <p className="font-black text-[#7C67B2] text-lg">{plan.name}</p>
                  <p className="text-sm text-[#273068]">
              ...
              {endsAt && (
                <>
                  <div className="flex justify-between text-sm mb-2">
                    <span className="text-[#273068]">Expiration</span>
                    <span className="font-semibold text-[#273068]">
                      {new Date(endsAt).toLocaleDateString("fr-FR")}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm mb-3">
                    <span className="text-[#273068]">Jours restants</span>
                    <span className={`font-black ${daysLeft !== null && daysLeft <= 7 ? "text-[#E94E6F]" : "text-[#6F8D4C]"}`}>
                      {daysLeft} jour{daysLeft !== 1 ? "s" : ""}
                    </span>
                  </div>
                  <div className="w-full bg-[#D5CDE2] rounded-full h-2 mb-5">
                    <div
                      className="h-2 rounded-full bg-[#7C67B2] transition-all"
              ...
              {startsAt && (
                <p className="text-xs text-[#273068] mb-5">
              ...
              <Link
                to="/abonnements"
                className="block w-full py-3 text-center bg-[#7C67B2] text-white rounded-xl font-semibold text-sm hover:bg-[#6a58a0] transition-colors"
              >
          ...
          <div className="max-w-md mx-auto text-center py-10">
            <div className="bg-[#FFFEFA] rounded-2xl p-8 shadow-sm">
              <p className="text-4xl mb-4">🌟</p>
              <h3 className="text-xl font-black text-[#7C67B2] mb-2">Plan gratuit</h3>
              <p className="text-sm text-[#273068] mb-6">
